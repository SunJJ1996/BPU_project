...
local_layer parse_local(list *options, size_params params)
{
    ...
    //读取cfg文件中的空洞卷积膨胀系数，如果没有则为1
    int size = 1;
    int dilated_parameter = option_find_int(options, "dilated_parameter",1);
    //若膨胀系数为1，则正常进行卷积
    //若膨胀系数不为1，则将size=size+(size-1)*(dilated_parameter-1)
    if (dilated_parameter == 1){
        size = option_find_int(options, "size",1);
    }
    else{
        size = option_find_int(options, "size",1);
        size = size + (size - 1) * (dilated_parameter - 1);
    }
    //dilated改变卷积核大小操作完成
    // int size = option_find_int(options, "size",1);
    int stride = option_find_int(options, "stride",1);
    //当dilated_para不为1时，pad的大小等于dilated_para的大小
    int pad = option_find_int(options, "pad",0);
    if(dilated_parameter != 1){
        pad = dilated_parameter;
    }
...
convolutional_layer parse_convolutional(list *options, size_params params)
{
    ...
    //下面make函数添加膨胀系数位
    convolutional_layer layer = make_convolutional_layer(batch,1,h,w,c,n,groups,size,stride_x,stride_y,dilation,padding,activation, batch_normalize, binary, xnor, params.net.adam, use_bin_output, params.index, antialiasing, share_layer, assisted_excitation, deform, params.train, dilated_parameter);
...
layer parse_crnn(list *options, size_params params)
{
    ...
    //若膨胀系数不为1，则将size=size+(size-1)*(dilated_parameter-1)
    if (dilated_parameter == 1){
        size = option_find_int_quiet(options, "size", 3);
    }
    else{
        size = option_find_int(options, "size",1);
        size = size + (size - 1) * (dilated_parameter - 1);
...
layer parse_normalization(list *options, size_params params)
{
    ...
    //若膨胀系数不为1，则将size=size+(size-1)*(dilated_parameter-1)
    if (dilated_parameter == 1){
        size = option_find_int(options, "size", 5);
    }
    else{
        size = option_find_int(options, "size", 5);
        size = size + (size - 1) * (dilated_parameter - 1);
